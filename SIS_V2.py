...
class SIS:
    ...
    def fetch_data(self):
        connect = pymysql.connect(host="localhost", user="root", password="", database="sis_v2")
        cursor = connect.cursor()
        cursor.execute("SELECT students.ID_Number, students.Full_Name,  courses.course_code, students.Year_Level, students.Gender FROM students INNER JOIN courses ON courses.course_code = students.Course ORDER BY students.ID_Number")
        rows = cursor.fetchall()
        if len(rows) != 0:
