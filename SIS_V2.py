...
class SIS:
    ...
    def __init__(self, root):
        ...
        self.add_student = Button(self.first_frame, text='Add', command=self.add_student, font=('Century Gothic', 10),
                                  bg='#ffffff', relief=GROOVE)
        self.add_student.grid(padx=5, pady=0, ipady=2, row=0, column=5)

        self.update_student = Button(self.first_frame, text='Update', command=self.update_student,
                                     font=('Century Gothic', 10), bg='#ffffff', relief=GROOVE)
        ...
        self.title = Label(self.below_frame, text='STUDENT INFORMATION SYSTEM', font=('Century Gothic', 17, 'bold'),
                           bg='#fb8263', fg='white')
        self.title.grid(padx=0, pady=2, ipady=2, row=0, column=1, sticky=W)
    ...
    def add_student(self):
        ...
        self.Label_course_entry['values'] = self.data
    ...
    def update_student(self):
        ...
        if not answer:
            ...
            self.Update = Button(self.Second_Frame, text='Update', font=('Century Gothic', 12), command=self.add,
                              bg='#fb8263', relief=GROOVE)
    ...
    def update(self):
        if (len(self.ID_Number.get()) == 0 or len(self.Full_Name.get()) == 0 or len(
            ...
            connect = pymysql.connect(host="localhost", user="root", password="", database="sis_v2")
            cursor = connect.cursor()
            cursor.execute("update students set Full_Name=%s, Course=%s, Year_Level=%s, Gender=%s where ID_Number=%s",(
                                                                                    self.Full_Name.get(),
                                                                                    self.Course.get(),
                                                                                    self.Year_Level.get(),
                                                                                    self.Gender.get(),
